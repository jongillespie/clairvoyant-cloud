...
// WiFi parameters & BLYNK Auth Code
const char WIFI_SSID[] = "iNet";
const char WIFI_PSK[] = "jg123456";
const char BLYNK_AUTH[] = "7fda75ccf9a1474d9b8d7eae2f04f1d6";
...
// Location data (Waterford Institute of Technology, Waterford Ireland)
const float LATITUDE = 52.245065;
const float LONGITUDE = -7.140286;
...
const int BUFFER_SIZE = 80; // bytes  /// was 22 /// 
...
// Set up serial console to read web page
  Serial.println("  ");
  Serial.println("Initialising the Serial Connection... :) ");
// TESTING NEW FEATURE

  // Start Blynk and wait for connection
  Blynk.begin(BLYNK_AUTH, WIFI_SSID, WIFI_PSK);
  while ( Blynk.connect() == false ) {
    led ^= 0x01;                                  // ^ is the XOR (exclusive OR)
...
// **changed to every 15 minutes, pull weather data and update LEDs
void doWeather() {

// IT DOES THIS OKAY. LOOPING LIKE MAD.

  // Only update every 15 minutes (900000s) // changed to 30 secs for testing.
...
//  unsigned long sunset = 0;
  ...
  String cmd = "GET /data/2.5/forecast?lat=";  // Was:  String cmd = "GET /data/2.5/weather?lat=";
  cmd += String(LATITUDE);
  cmd += "&lon=";
  cmd += String(LONGITUDE);
  cmd += "&appid=66e9c37edcfd4f6a877c6d05d4bba9ac";  // Weather API KEY
  ...
  //
Serial.println("  ");
Serial.println("Used Weather API KEY ");
Serial.println("Printing CMD ");
Serial.println(cmd);
//   
...
//    
      ...
      time = millis();  // FIND OUT if the millis() is the same as current UNIX time. NOT. Its from 'power on'
    }
    
    // Depending on the state of the parser, look for string
    switch ( parser_state ) {
      case PARSER_START:
        if ( memcmp(buf, "\"weather\":", 10) == 0 ) {  // compares the blocks and if equal (0) true

//
Serial.println("  ");
Serial.println("This is: buf 1 ");
Serial.println(buf);
//            
      ...
      case PARSER_WEATHER:
        if ( memcmp(buf, "\"icon\":\"", 8) == 0 ) {
          ...
          memcpy(icon, buf + 8, 3); // copies 3 bytes from buffer to icon for later use.

//
Serial.println("  ");
Serial.println("This is: buf 2 ");
Serial.println(buf);
//   
